...
/**
 * Hero streams from YouTube. User taps Play → title exits → video. After scrolling past
 * the viewport and returning to top, intro + Play return.
 * @see https://youtu.be/xvInodQv6q0
 */
...
/** Back at top of document — show intro again (after having left hero while video was playing) */
...
export default function HeroSection() {
  ...
  /** Bump to replay title entrance CSS after returning from video */
  const [introAnimKey, setIntroAnimKey] = useState(0);
  /** Remount iframe after reset */
  ...
  return (
    <section
      id="hero"
      className="relative h-svh max-h-svh min-h-0 w-full shrink-0 overflow-hidden bg-[#0a0a0a]"
    >
      <div className="absolute inset-0 z-0">
        <div
          aria-hidden
          className="absolute inset-0 bg-cover bg-center transition-[opacity,filter] duration-300 ease-out"
          style={{
            backgroundImage: `url(${HERO_POSTER})`,
            opacity: posterUnderYt ? 0.35 + scrollFade * 0.5 : heroPhase === 'video' ? 0.95 : 0.85,
        ...
        {heroPhase === 'video' && ytEmbedSrc && !ytUnavailable && !ytSuspended && (
          <div
            className="absolute inset-0 overflow-hidden transition-[opacity,filter] duration-300 ease-out"
            style={{
        ...
        {heroPhase === 'video' && ytUnavailable && (
          <div className="absolute inset-0 z-[18] flex flex-col items-center justify-center gap-5 bg-black/60 px-6 text-center backdrop-blur-[2px]">
        ...
        <div
          className="pointer-events-none absolute inset-0 bg-gradient-to-b from-black/50 via-black/25 to-black/60"
      ...
      {heroPhase === 'intro' && (
        <>
          <div
            key={introAnimKey}
            className="hero-titles pointer-events-none absolute inset-0 z-20 flex h-full min-h-0 flex-col justify-between p-8 pb-28 md:p-16 md:pb-32 hero-titles--in"
          >
      ...
      {heroPhase === 'exit' && (
        <div className="hero-titles pointer-events-none absolute inset-0 z-20 flex h-full min-h-0 flex-col justify-between p-8 pb-28 md:p-16 md:pb-32 hero-titles--out">
          <h1 className="hero-title-line hero-title-line--left display-heading text-bone text-[12vw] md:text-[8vw] leading-none drop-shadow-[0_2px_24px_rgba(0,0,0,0.45)]">
            PNEUMI
          </h1>
          <div className="flex flex-col items-end">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 bottom-0 z-[2] h-32 bg-gradient-to-b from-transparent via-[#FBFBF9]/70 to-[#FBFBF9] transition-opacity duration-300 md:h-44"
